...
<template>
  <div class="grid grid-cols-1 gap-4">
    ...
    <div
      ...
      <div
        class="relative z-10 w-full h-full p-4 pt-10 grid grid-cols-4 gap-x-3 gap-y-4 auto-rows-max place-items-center">
        <!-- Generated Icon -->
        ...
        <div class="flex flex-col items-center gap-1 opacity-90" v-for="i in 3" :key="i">
          <div
            class="w-11 h-11 rounded-[11px] bg-white/30 backdrop-blur-md shadow-sm border border-white/20 flex items-center justify-center text-white">
            <Box :size="20" />
          </div>
      ...
      <!-- iOS Dock -->
      <div
        class="absolute bottom-4 left-3.5 right-3.5 h-[56px] rounded-2xl bg-white/30 backdrop-blur-2xl border border-white/20 z-10 flex items-center justify-around px-2.5 shadow-[0_6px_20px_rgba(0,0,0,0.1)]">
        <div class="w-10 h-10 rounded-[11px] bg-[#34c759] flex items-center justify-center text-white shadow-sm">
          <MessageCircle :size="20" />
        </div>
        <div class="w-10 h-10 rounded-[11px] bg-[#007aff] flex items-center justify-center text-white shadow-sm">
          <Mail :size="20" />
        </div>
        <div class="w-10 h-10 rounded-[11px] bg-[#ff3b30] flex items-center justify-center text-white shadow-sm">
          <Music :size="20" />
        </div>
        <div class="w-10 h-10 rounded-[11px] bg-[#ff9500] flex items-center justify-center text-white shadow-sm">
          <Compass :size="20" />
        </div>
      </div>

      <!-- Home Indicator -->
      <div class="absolute bottom-1.5 left-1/2 -translate-x-1/2 w-1/3 h-0.5 bg-white/90 rounded-full z-20 shadow-sm">
      </div>
    </div>
    ...
    <div
      ...
      <!-- Camera Hole -->
      <div class="absolute top-2.5 left-1/2 -translate-x-1/2 w-3.5 h-3.5 bg-[#2d3430] rounded-full z-20 shadow-inner">
      </div>

      <!-- Android Status Bar -->
      <div
        class="absolute top-0 w-full h-7 px-4 flex justify-between items-center z-20 text-white text-[10px] font-bold tracking-wide">
        <span>10:00</span>
      ...
      <!-- Search Widget -->
      <div
        class="absolute top-10 left-3.5 right-3.5 h-10 bg-white/95 rounded-full z-10 flex items-center px-3.5 shadow-[0_2px_8px_rgba(0,0,0,0.08)] backdrop-blur-md text-[#757c77]">
      ...
      <div
        class="relative z-10 w-full h-full p-4 pt-24 grid grid-cols-4 gap-x-3 gap-y-4 auto-rows-max place-items-center">
        <!-- Dummy Icons -->
        <div class="flex flex-col items-center gap-1 opacity-90" v-for="i in 3" :key="i">
          <div
            class="w-11 h-11 rounded-full bg-white/30 backdrop-blur-md shadow-sm border border-white/20 flex items-center justify-center text-white">
            <Hexagon :size="20" />
          </div>
        ...
        <div class="flex flex-col items-center gap-1 group cursor-pointer hover:scale-95 transition-transform">
          <div
            class="w-11 h-11 shadow-md overflow-hidden bg-white/10 flex items-center justify-center relative rounded-full">
      ...
      <!-- Android Dock -->
      <div
        class="absolute bottom-5 left-3.5 right-3.5 h-[56px] bg-white/30 backdrop-blur-2xl rounded-full border border-white/20 z-10 flex items-center justify-around px-3 shadow-[0_6px_20px_rgba(0,0,0,0.1)]">
        <div class="w-9 h-9 rounded-full bg-[#007aff] flex items-center justify-center text-white shadow-sm">
          <Phone :size="18" />
        </div>
        <div class="w-9 h-9 rounded-full bg-[#34c759] flex items-center justify-center text-white shadow-sm">
          <MessageCircle :size="18" />
        </div>
        <div class="w-9 h-9 rounded-full bg-[#ff3b30] flex items-center justify-center text-white shadow-sm">
          <Mail :size="18" />
        </div>
        <div class="w-9 h-9 rounded-full bg-[#ff9500] flex items-center justify-center text-white shadow-sm">
          <Camera :size="18" />
        </div>
        <div class="w-9 h-9 rounded-full bg-slate-500 flex items-center justify-center text-white shadow-sm">
          <Settings :size="18" />
        </div>
      </div>
