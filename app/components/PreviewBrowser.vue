...
<template>
  <div
    ...
    <!-- Browser Top Bar -->
    <div class="h-12 bg-[#fdfbf7] flex items-center px-3 sm:px-4 gap-2 select-none border-b border-[#e4e2de]">
      <!-- Traffic Lights -->
      ...
      <div class="flex-1 flex px-2 sm:px-4 mt-2 h-10 items-end overflow-hidden">
        <!-- Active Tab -->
        <div
          class="h-8 min-w-[120px] max-w-[160px] sm:min-w-[160px] sm:max-w-[200px] bg-white rounded-t-lg flex items-center px-2.5 sm:px-3 gap-2 shadow-[0_-2px_6px_rgba(45,52,48,0.05)] border-x border-t border-[#e4e2de] relative z-20">
        ...
        <!-- Dark Tab -->
        <div
          class="h-8 min-w-[120px] max-w-[160px] sm:min-w-[160px] sm:max-w-[200px] bg-[#202124] rounded-t-lg flex items-center px-2.5 sm:px-3 gap-2 shadow-[0_-2px_6px_rgba(0,0,0,0.15)] border-x border-t border-[#3c4043] relative z-10">
          <img v-if="generatedIcon" :src="generatedIcon" class="w-4 h-4 object-contain" alt="Favicon" />
        ...
        <!-- Inactive Tab -->
        <div
          class="hidden h-8 min-w-[140px] max-w-[180px] items-center px-3 gap-2 opacity-50 transition-opacity hover:opacity-80 md:flex">
    ...
    <div class="h-10 bg-white border-b border-[#e4e2de] flex items-center px-3 sm:px-4 gap-2 sm:gap-4">
      <div class="flex gap-1.5 sm:gap-2 text-[#acb4ae]">
        <ChevronLeft :size="16" />
        <ChevronRight :size="16" />
        <RotateCw :size="16" />
      </div>
      <div class="flex-1 h-6 bg-[#fdfbf7] border border-[#e4e2de] rounded-md flex items-center px-2.5 sm:px-3 justify-center min-w-0">
        <span class="text-[10px] sm:text-[11px] font-medium text-[#757c77] truncate">iconbase.7-image.com</span>
      </div>
    </div>
  </div>
</template>
