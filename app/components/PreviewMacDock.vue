...
<template>
  <div class="h-48 rounded-xl overflow-hidden shadow-sm border border-gray-200 transition-colors relative flex flex-col justify-end bg-[url('https://coresg-normal.trae.ai/api/ide/v1/text_to_image?prompt=beautiful%20macOS%20wallpaper%2C%20abstract%20gradient%20landscape%2C%20high%20detail&image_size=landscape_16_9')] bg-cover bg-center">
    ...
    <div class="w-full flex justify-center pb-2 relative z-10">
      <!-- Dock Container -->
      <div class="relative z-10 mb-4 px-4 py-2 self-center bg-white/20 backdrop-blur-2xl border border-white/20 rounded-[2rem] flex items-center gap-3 shadow-xl">
        <!-- Dock Items -->
        <div class="w-14 h-14 bg-gradient-to-br from-gray-100 to-gray-300 rounded-[1.2rem] shadow-sm flex items-center justify-center shrink-0">
          <Settings :size="28" class="text-gray-600" />
        </div>
        ...
        <div class="w-14 h-14 bg-white rounded-[1.2rem] shadow-sm flex flex-col items-center justify-center shrink-0 border border-gray-200">
          <span class="text-[8px] font-bold text-red-500">JUL</span>
          <span class="text-xl font-light leading-none mt-0.5">17</span>
        </div>
      </div>
    </div>
  </div>
